...
export default function StudentBubbleChart({
  ...
  const handleTimeChart = (current) => {
    ...
    timeArr?.map((curr, id) => {
      const x = (id + 1) * 5;
      const y = curr?.avgTimeTaken>0 ? (curr?.avgTimeTaken/100)?.toFixed(2) : 0;
        ...
        tooltip: {
          callbacks: {
            label: function (context, c, d) {
              return `  ${"Time "} : ${(context.parsed.y / 60)?.toFixed(
                2
              )} minutes`;
            },
          },
      ...
      scales: {
        ...
        x: {
          ...
          ticks: {
            ...prev.scales.x.ticks,
            callback: function (value, index, ticks) {
              return concepts[index - 1]?.length > 10
                ? concepts[index - 1]?.slice(0, 10)
                : concepts[index - 1];
            },
          },
        },
