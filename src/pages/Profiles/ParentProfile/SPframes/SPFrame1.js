...
const SPFrame1 = ({
      ...
      getSessions(userDetail?.userId).then((response) => {
        ...
        response?.data?.sessionNotes?.map((it) => {
          if (it?.clientNotes?.note)
            setClientNotes((prev) => {
              return [...prev, { ...it?.clientNotes }];
            });
          if (it?.internalNotes?.note)
            setInternalNotes((prev) => {
              return [...prev, { ...it?.internalNotes }];
            });
        });
      });
    }
  ...
  if (persona === "student" || persona === "parent") return <></>;
                  ...
                  <div className=" !w-[calc(545*0.0522vw)]">
                    ...
                    <div className="bg-white flex-1 text-base-17-5 p-3 text-[#B5B5B5] h-fit max-h-[210px] overflow-y-auto custom-scroller rounded-md shadow-[0px_0px_2.500001907348633px_0px_#00000040]">
                      {user?.internalNotes?.length > 0 ? (
                        user?.internalNotes?.map((item, index) => (
                          <>
                            <div
                              key={index}
                              className="flex h-[57px] relative items-center"
                            >
                          ...
                          new Date(item.date)
                            ...
                            new Date(item.date)
                              ...
                              <div
                                className={` ${styles.actionBorder} items-center`}
                              >
                        ...
                        <>
                          {" "}
                          Add notes about the parent. Here are some ideas to get
                          you started:
                    ...
                    <div className="bg-white flex-1 h-[400px] p-3 custom-scroller  overflow-y-auto  flex flex-col rounded-md shadow-[0px_0px_2.500001907348633px_0px_#00000040] custom-scroller">
                      {signupData?.map((it, id) => {
                        return (
                          <p
                            key={id}
                            className="flex gap-3 my-2 text-base-17-5"
                          >
                      ...
                      {internal &&
                        internalNotes?.map((item, index) => (
                          <>
                            <div
                              key={index}
                              className="flex h-[57px] pl-5 relative items-center"
                            >
                              <p className="text-[#517CA8]  !font-medium text-[14px] mr-4 w-[calc(143*0.050vw)] text-center !text-[calc(17.5*0.050vw)] whitespace-nowrap">
                                {item?.note &&
                                  new Date(item.date)
