...
export default function EventModal({
  ...
  const [updateAllUserSession, updateAllUserSessionResp] =
    useUpdateAllSessionMutation();
  const [updateSessionStatus, updateSessionStatusResp] =
    useLazyUpdateSessionStatusQuery();
  const [submitFeedback, submitFeedbackResp] = useSubmitFeedbackMutation();
  const [getSessionFeedback, getSessionFeedbackResp] =
    useLazyGetSessionFeedbackQuery();
  ...
  const [deleteAllSession, deleteAllSessionResp] =
    useDeleteAllRecurringSessionMutation();
  ...
  const [notesType, setNotesType] = useState("clients");
  ...
  useEffect(() => {
    if (organization?.settings) {
      if (
        ...
        organization?.settings?.permissions[5]?.choosedValue === false
      ) {
  ...
  useEffect(() => {
    if (isUpdating) {
      setIsProductive(
        getCheckedItems([sessionToUpdate.sessionProductive], isProductive)
      );
  ...
  const updateSession = async (reqBody, isUpdaingAll, sDate) => {
    ...
    if (body.sessionStatus === "Completed") {
      await updateSessionStatus(sessionToUpdate._id).then((res) => {
        if (res.error) return;
    ...
    if (body.sessionStatus === "Missed") {
      await missSession(sessionToUpdate._id).then((res) => {
        if (res.error) {
    ...
    if (body.sessionStatus === "Cancelled") {
      await cancelSession(sessionToUpdate._id).then((res) => {
        if (res.error) {
    ...
    if (isUpdaingAll === true) {
      updateAllUserSession({
        id: sessionToUpdate._id,
        body: { ...body, sessionStatus: "Scheduled" },
      }).then((res) => {
        console.log(res);
        setLoading(false);
        refetchSessions();
      ...
      updateUserSession({
        id: sessionToUpdate._id,
        body: { ...body, _id: sessionToUpdate._id },
      }).then((res) => {
        console.log(res);
        refetchSessions();
        setEventModalActive(false);
  ...
  const handleSubmit = (isUpdatingAll) => {
    ...
    if (offset > 0) {
      const endDateUpdated = new Date(
        new Date(reqBody.endDate).getTime() + offset
      );
      ...
      if (daysTORecur.length > 0) {
        daysTORecur.map((recurDay) => {
          if (currentDay === recurDay) {
            ...
            let d2 = sDate;
            let up2 = new Date(d2).setDate(
              currentDate + 7 + recurDay - currentDay
            );
    ...
    // setLoading(false)
    // return
    if (isUpdating && isUpdatingAll)
      return updateSession(reqBody, isUpdatingAll, sDate);
  ...
  const fetchFeedback = () => {
    getSessionFeedback(sessionToUpdate._id).then((res) => {
      if (res.error) {
        setInputFeedback(0);
        return console.log(res.error);
      }
  ...
  const handleDeleteSession = () => {
    setLoading(true);
    deleteSession(sessionToUpdate._id).then((res) => {
      setLoading(false);
  ...
  const handleDeleteAllSession = () => {
    setLoading(true);
    deleteAllSession(sessionToUpdate._id).then((res) => {
      setLoading(false);
      if (res.error) return console.log(res.error);
      console.log(res.data);
      refetchSessions();
  ...
  useEffect(() => {
    ...
    getUserDetail({ id: data.tutorId }).then((res) => {
      if (res.error) {
      ...
      if (details?.tutorServices?.length === 0)
        return alert("Tutor does not have any services");
  ...
  useEffect(() => {
    if (isUpdating) {
      ...
      if (
        initialTime.start.time === changedTime.start.time &&
    ...
    isUpdating,
    sessionToUpdate?.time,
    data?.time,
    sessionToUpdate?.date,
    data?.date,
  ]);
      ...
      <Modal
        ...
        title={
          isEditable === false
            ...
            ? "Update Session"
            : ` ${
                persona == "tutor" ? "Session Details" : "Schedule New Session"
              }`
        }
        body={
          <>
                ...
                <div className="flex mb-7">
                  <InputSelect
                    ...
                    onChange={(val) => {
                      // console.log(val)
                      data.service !== val &&
                        setData({ ...data, service: val, specialization: "" });
                    }}
                    optionData={servicesAndSpecialization}
                    inputContainerClassName={`bg-lightWhite pt-3.5 pb-3.5 border-0 font-normal pr-3 text-[#B3BDC7]
                       `}
                    inputClassName="bg-transparent appearance-none font-normal pt-4 pb-4 text-[#B3BDC7]"
                    placeholder="Select"
                  ...
                  <>
                    <div className="mt-7 mb-5 w-full  ">
                      {allSessionTags.map((tag) => {
                        return (
                          <div key={tag._id}>
                            <p className="font-medium mb-2.5">{tag.heading}</p>
                            <div className="flex !flex-wrap gap-3">
                              {tag.items.length > 0 &&
                                tag.items.map((item, idx) => {
                                  const currentUserSession =
                                    data.sessionTags.find(
                                      (dataSessionTag) =>
                                        dataSessionTag._id === tag._id
                                    );
                                  let checked = false;
                                  if (
                                    currentUserSession?.items.includes(item)
                                  ) {
                                    checked = true;
                                  }
                                  return (
                                    <div
                                      ...
                                      onClick={() => {
                                        if (stopUpdating)
                                          handleSessiontagChange(item, tag._id);
                                      }}
                                    >
                                      <CCheckbox
                                        checked={checked}
                                        name="topic"
                                      />
                                      <p className="font-medium text-[#507CA8] text-sm">
                                        {item}
                                      </p>
                                    </div>
                                  );
                    ...
                    {persona == "admin" ? (
                      <div className="h-[300px] flex flex-col justify-start items-start">
                        <div className="h-[50px] w-[285.33px] flex items-center justify-between border-b-[1.33px] border-[#6009EF]">
                          <button
                            type="button"
                            className={`text-[18.66px] font-normal py-2 text-left ${
                              notesType === "clients"
                                ? "text-[#FFA28D] border-b-4 border-[#FFA28D]"
                                : "text-[#26435F]"
                            }`}
                            onClick={() => {
                              setNotesType("clients");
                            }}
                          >
                            Client Notes
                          </button>
                          ...
                          <button
                            ...
                            onClick={() => {
                              setNotesType("internal");
                            }}
                          >
                            Internal Notes
                          </button>
                        </div>
                        <div className=" w-full min-h-[84px] flex flex-1 flex-grow-1 justify-center items-center">
                          {notesType === "clients" ? (
                            <textarea
                              ...
                              rows={3}
                              className="bg-lightWhite w-full h-full outline-0 px-5 py-4 rounded"
                            ></textarea>
                          ) : (
                            <textarea
                              placeholder="Session notes that are added in this box will be visible to clients (parents and students). If you want to add notes that are hidden from them, please use Internal Notes."
                              ...
                              rows={3}
                              className="bg-lightWhite w-full outline-0 px-5 py-4 rounded h-full"
                            ></textarea>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="mb-8">
                        ...
                        ></textarea>
                        <p className="text-right text-xs text-primary-80">
                          {data.sessionNotes ? data.sessionNotes.length : "0"}
                          /200
                        </p>
                      </div>
                    )}
                    ...
                    {[...Array(5)].map((x, i) => (
                      <img
                        ...
                        className={`mr-7 cursor-pointer ${
                          data.sessionStatus?.toLowerCase() !== "completed"
                            ? "pointer-events-none"
                            : ""
                        } `}
                        onClick={() => {
                  ...
                  body={
                    <>
                      <p className="text-base-17-5 mt-[-5px] text-[#667085] mb-10">
                        ...
                        All deleted session data will be lost and you will NOT
                        be able to recover it later. Note that this might also
                        impact the Client's digital wallet accordingly. Read
