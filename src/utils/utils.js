...
export function tConvert(time) {
  ...
  return {
    time: `${time[0] < 10 ? "0" : ""}${time
      .join("")
      .substring(0, time.length - charsToSubtract)}`,
    timeType: type,
  }; // return adjusted time or original string
}
export function getToTimezone(val) {
  ...
  if (timeZoneAbbreviations[val]) return timeZoneAbbreviations[val];
  return "";
}
...
export const convertDateToTimezone = (
  ...
  const startTime = convertTime12to24(`${time} ${timeType}`);
