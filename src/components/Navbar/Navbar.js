...
const [loading2,setLoading2]=useState(false)
  ...
  return (
    <>
      <div
        className={`flex   bg-[#26435F] h-[72px] items-center w-full shadow-[0px_4px_4px_0px_rgba(0,0,0,0.25)] ${
          persona == "superAdmin" ? "justify-between px-[5%]" : "justify-around"
        }`}
      >
        <div
          className={`${
            persona === "superAdmin" ? "translate-x-[-2.3vw]" : ""
          }`}
        >
          <Link to="/">
            <div className="flex gap-x-[6px] items-center">
              ...
              />
              <p
                className={`text-[43px] text-white font-bold pt-[3.2px] ${styles.customFont}`}
              >
                Evallo
              </p>
            </div>
        ...
        <div
          className={`flex  text-[#FFFFFF] font-semibold text-[0.9021vw] ${
            !isLoggedIn && "opacity-[0.3]"
          } ${persona === "superAdmin" ? "translate-x-[3.5vw]" : ""}`}
        >
          {navData.map((item, idx) => {
            return (
              <div
                key={idx}
                className={`flex items-center mr-6 design:mr-10  ${
              ...
              >
                {isLoggedIn && item?.path === activeRoute ? (
                  <>
                    ...
                    <p className="pl-[10px] text-[#FFA28D] text-text-[0.902vw] ">
                      {" "}
                      {item.tooltip}{" "}
                    </p>
                  </>
                  ...
                  <>
                    ...
                    <p className="pl-[10px] text-[0.902vw] ">
                      {" "}
                      {item.tooltip}{" "}
                    </p>
                  </>
        ...
        <div
          className={`flex ${
            persona === "superAdmin" ? "translate-x-[-1vw]" : ""
          } font-bold ${isLoggedIn ? "" : "opacity-[0.3]"}`}
        >
          {persona == "parent" || persona == "tutor" || (
            <div className="cursor-pointer flex mr-[24px] text-[#24A3D9] text-base-16 items-center">
              <p className=" text-[0.83vw]">Pricing</p>
              <p className="pl-2 text-[17px]">$</p>
            </div>
          )}
          ...
          <div className="cursor-pointer flex mr-[24px] text-[#24A3D9] items-center text-base-16 ">
            ...
            <p>
              <img className=" ml-2" src={faQuestionCircle} alt="" />
            </p>
          </div>
          ...
          >
            <div>
              <p className="text-[#24A3D9] text-[0.83vw] !font-semibold">
                Logout
              </p>
            </div>
            <div>
              <img className=" ml-2" src={logoutIcon} alt="" />
            </div>
          </div>
        </div>
      </div>
      {logoutModalActive && (
        <Modal
          ...
          primaryBtn={{
            ...
            loading: loading2,
            className:
              "text-base bg-[#FF7979] !w-[146px] pl-4 pr-4   !rounded-[8px] font-medium !mr-auto !text-center !bg-[#FF7979] !h-[46px]",
            onClick: logoutUser,
          }}
          handleClose={() => setLogoutModalActive(false)}
          body={<div className="mb-6"></div>}
          classname={
            "!w-[666px] mx-auto !pt-7 !pb-[33px] !rounded-[8px] px-[33.33px] !text-center"
          }
        />
      )}
    </>
