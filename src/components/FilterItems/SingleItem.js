...
export default function SingleItem({ item, image, idx, className, onRemoveFilter, keyName, sliceText, isString, fetchData,baseLink }) {
   ...
   return (
      <div key={idx} className={`mr-3 ${className ? className : ''} bg-primaryLight py-1 px-3 rounded-7 group ${styles.filterItem}`}>
         {image && <div className='w-[85px] h-[85px] mx-auto flex justify-center'>
            <img src={`${baseLink}${item.image}`} className='w-full h-full object-contain	' />
         </div>}
