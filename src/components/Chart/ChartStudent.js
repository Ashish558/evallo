...
import { backgroundColors, iniOptionsStudent } from './data';
...
export default function ChartStudent({ setSubjects, YHeader, subjects, selectedSubject, selectedStudent, currentSubData, setCurrentSubData, selectedConceptIdx }) {

    const [fetchPersonalDetails, personalDetailsResp] = useLazyGetPersonalDetailQuery()
    const [options, setOptions] = useState(iniOptionsStudent)
    ...
    return (
        data !== undefined &&
        <div className='wrapper w-full min-w-2/3 overflow-x-auto h-[382px]  pl-[30px] relative'  >
            ...
            <p className='absolute bottom-[45%] right-[93%]'><img src={downArrow} alt="" /></p>
        </div>
    )
}
